...
void JetAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    for (unsigned int ptIdx = 0; ptIdx < selectedJets.size(); ++ptIdx) {
		...
		const std::vector<const pat::PackedCandidate> pfsVector = *pfs;
		...
		std::sort(pfsVector.begin(), pfsVector.end(), [](const pat::PackedCandidate &p1, pat::PackedCandidate &p2) {return p1.pt() > p2.pt(); });
		nPFR=0;
