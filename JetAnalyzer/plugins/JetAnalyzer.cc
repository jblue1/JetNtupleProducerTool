...
JetAnalyzer::JetAnalyzer(const edm::ParameterSet &iConfig)
    : vtxToken_(consumes<reco::VertexCollection>(
      ...
      jetToken_(consumes<pat::JetCollection>(
      ...
      EDMGenJetsToken_(consumes<reco::GenJetCollection>(
          iConfig.getParameter<edm::InputTag>("genJets"))),
      genEventInfoToken_(consumes<GenEventInfoProduct>(
          iConfig.getParameter<edm::InputTag>("genEventInfo"))),
      pileupInfoToken_(consumes<std::vector<PileupSummaryInfo>>(
      ...
      pfRhoAllToken_(
          consumes<double>(iConfig.getParameter<edm::InputTag>("pfRhoAll"))),
      pfRhoCentralToken_(consumes<double>(
          iConfig.getParameter<edm::InputTag>("pfRhoCentral"))),
      pfRhoCentralNeutralToken_(consumes<double>(
          iConfig.getParameter<edm::InputTag>("pfRhoCentralNeutral"))),
      pfRhoCentralChargedPileUpToken_(consumes<double>(
          iConfig.getParameter<edm::InputTag>("pfRhoCentralChargedPileUp"))),
      qglToken_(consumes<edm::ValueMap<float>>(
          edm::InputTag("QGTagger", "qgLikelihood"))),
      ptDToken_(
          consumes<edm::ValueMap<float>>(edm::InputTag("QGTagger", "ptD"))),
      axis2Token_(
          consumes<edm::ValueMap<float>>(edm::InputTag("QGTagger", "axis2"))),
      multToken_(
          consumes<edm::ValueMap<int>>(edm::InputTag("QGTagger", "mult"))) {
...
void JetAnalyzer::beginJob() {
  ...
  pfCandTree->Branch("pfCandVx", &pfCandVx);
  pfCandTree->Branch("pfCandVy", &pfCandVy);
  pfCandTree->Branch("pfCandVz", &pfCandVz);
  ...
  jetTree->Branch("jetChargedHadronMult", &jetChargedHadronMult,
                  "jetChargedHadronMult/I");
  jetTree->Branch("jetNeutralHadronMult", &jetNeutralHadronMult,
                  "jetNeutralHadronMult/I");
  ...
  jetTree->Branch("genJetPF_dTheta", &genJetPF_dTheta,
                  "genJetPF_dTheta[nGenJetPF]/F");
  jetTree->Branch("genJetPF_mass", &genJetPF_mass,
                  "genJetPF_mass[nGenJetPF]/F");
  ...
  jetTree->Branch("rhoCentralNeutral", &rhoCentralNeutral,
                  "rhoCentralNeutral/F");
  jetTree->Branch("rhoCentralChargedPileUp", &rhoCentralChargedPileUp,
                  "rhoCentralChargedPileUp/F");
  ...
  genPartTree->GetBranch("genPartPdgId")->SetTitle("pdgId of parton");
  genPartTree->GetBranch("genPartStatus")
      ->SetTitle("Pythia8 status code of parton");
  genPartTree->GetBranch("genPartPt")
      ->SetTitle("Transverse momentum of parton");
  genPartTree->GetBranch("genPartEta")->SetTitle("Eta of parton");
  genPartTree->GetBranch("genPartPhi")->SetTitle("Phi of parton");
  genPartTree->GetBranch("genPartM")->SetTitle("Mass of parton");
  genPartTree->GetBranch("genPartPx")
  ...
  genPartTree->GetBranch("genPartVy")
      ->SetTitle("y coordinate of vertex position of parton");
  genPartTree->GetBranch("genPartVz")
      ->SetTitle("z coordinate of vertex position of parton");
  ...
  jetTree->GetBranch("jetGirth")
      ->SetTitle("Girth of the jet (as defined in arXiv:1106.3076 [hep-ph])");
  jetTree->GetBranch("jetArea")->SetTitle(
      "Catchment area of the jet; used for jet energy corrections");
  jetTree->GetBranch("jetRawPt")
      ->SetTitle("Transverse momentum of the jet before energy corrections");
  jetTree->GetBranch("jetRawMass")
      ->SetTitle("Mass of the jet before energy corrections");
  jetTree->GetBranch("jetLooseID")
      ->SetTitle("Indicates if the jet passes loose selection criteria; used "
                 "for dismissing fake jets");
  jetTree->GetBranch("jetTightID")
      ->SetTitle("Indicates if the jet passes tight selection criteria; used "
                 "for dismissing fake jets");
  jetTree->GetBranch("jetGenMatch")
      ->SetTitle("1: if a matched generator level jet exists; 0: if no match "
                 "was found");
  jetTree->GetBranch("jetQGl")->SetTitle(
      "Quark vs Gluon likelihood discriminator");
  jetTree->GetBranch("QG_ptD")->SetTitle(
      "Transverse momentum distribution among particle flow candidates within "
      "the jet; defined as sqrt(Sum(pt^2))/Sum(pt), where the sum is over the "
      "particle flow candidates of the jet");
  jetTree->GetBranch("QG_axis2")
      ->SetTitle("Minor axis of the jet, calculated from the particle flow "
                 "candidates");
  jetTree->GetBranch("QG_mult")->SetTitle(
      "Multiplicity of jet constituents with additional cuts: 1 GeV transverse "
      "momentum threshold for neutral particles; charged particles required to "
      "be associated with the primary interaction vertex (PF_fromPV == 3)");
  jetTree->GetBranch("partonFlav")
      ->SetTitle("Flavor of the jet; parton definition");
  jetTree->GetBranch("hadronFlav")
      ->SetTitle("Flavor of the jet; hadron definition");
  jetTree->GetBranch("physFlav")
      ->SetTitle("Flavor of the jet; physics definition");
  jetTree->GetBranch("isPhysUDS")
      ->SetTitle("Indicates a light quark jet; |physFlav| == 1, 2, 3");
  jetTree->GetBranch("isPhysG")->SetTitle(
      "Indicates a gluon jet; physFlav == 21");
  jetTree->GetBranch("isPhysOther")
      ->SetTitle(
          "Indicates a non-light quark/gluon jet; |physFlav| != 1, 2, 3, 21");
  jetTree->GetBranch("isPartonUDS")
      ->SetTitle("Indicates a light quark jet; |physFlav| == 1, 2, 3");
  jetTree->GetBranch("isPartonG")
      ->SetTitle("Indicates a gluon jet; physFlav == 21");
  jetTree->GetBranch("isPartonOther")
      ->SetTitle(
          "Indicates a non-light quark/gluon jet; |physFlav| != 1, 2, 3, 21");
  jetTree->GetBranch("jetChargedHadronMult")
      ->SetTitle("Multiplicity of charged hadron jet constituents");
  jetTree->GetBranch("jetNeutralHadronMult")
      ->SetTitle("Multiplicity of neutral hadron jet constituents");
  jetTree->GetBranch("jetChargedMult")
      ->SetTitle("Multiplicity of charged jet constituents");
  jetTree->GetBranch("jetNeutralMult")
      ->SetTitle("Multiplicity of neutral jet constituents");
  jetTree->GetBranch("jetMult")->SetTitle("Multiplicity of jet constituents");
  jetTree->GetBranch("nPF")->SetTitle(
      "Number of particle flow candidates (particles reconstructed by the "
      "particle flow algorithm); contains all particles within |deltaPhi| < 1 "
      "&& |deltaEta| < 1 from the center of the jet");
  jetTree->GetBranch("PF_pT")->SetTitle(
      "Transverse momentum of a particle flow candidate");
  jetTree->GetBranch("PF_dR")->SetTitle(
      "Distance of a particle flow candidate to the center of the jet");
  jetTree->GetBranch("PF_dTheta")
      ->SetTitle("Polar angle of a particle flow candidate");
  jetTree->GetBranch("PF_dPhi")->SetTitle(
      "Azimuthal angle of a particle flow candidate");
  jetTree->GetBranch("PF_dEta")->SetTitle(
      "Pseudorapidity of a particle flow candidate");
  jetTree->GetBranch("PF_mass")->SetTitle("Mass of a particle flow candidate");
  jetTree->GetBranch("PF_id")->SetTitle(
      "Generator level particle identifier for the particle flow candidates, "
      "as defined in the PDG particle numbering scheme");
  jetTree->GetBranch("PF_fromPV")
      ->SetTitle("Indicates how tightly the particle is associated with the "
                 "primary vertex; ranges from 3 to 0");
  jetTree->GetBranch("PF_fromAK4Jet")
      ->SetTitle(
          "1: if the particle flow candidate is a constituent of the "
          "reconstructed AK4 jet; 0: if it is not a constituent of the jet");
  jetTree->GetBranch("genJetPt")
      ->SetTitle("Transverse momentum of the matched generator level jet");
  jetTree->GetBranch("genJetEta")
      ->SetTitle("Pseudorapidity of the matched generator level jet");
  jetTree->GetBranch("genJetPhi")
      ->SetTitle("Azimuthal angle of the matched generator level jet");
  jetTree->GetBranch("genJetMass")
      ->SetTitle("Mass of the matched generator level jet");
  jetTree->GetBranch("nGenJetPF")
      ->SetTitle("Number of particles in the matched generator level jet");
  jetTree->GetBranch("genJetPF_pT")
      ->SetTitle("Transverse momentum of a particle in the matched generator "
                 "level jet");
  jetTree->GetBranch("genJetPF_dR")
      ->SetTitle("Distance of a particle to the center of the matched "
                 "generator level jet ");
  jetTree->GetBranch("genJetPF_dTheta")
      ->SetTitle(
          "Polar angle of a particle in the matched generator level jet");
  jetTree->GetBranch("genJetPF_mass")
      ->SetTitle("Mass of a particle in the matched generator level jet");
  jetTree->GetBranch("genJetPF_id")
      ->SetTitle("Generator level particle identifier for the particles in the "
                 "matched generator level jet, as defined in the PDG particle "
                 "numbering scheme");
  jetTree->GetBranch("eventJetMult")
      ->SetTitle("Multiplicity of jets in the event");
  jetTree->GetBranch("jetPtOrder")
      ->SetTitle("Indicates the ranking number of the jet, as the jets are "
                 "ordered by their transverse momenta within the event");
  jetTree->GetBranch("dPhiJetsLO")
      ->SetTitle("Phi difference of the two leading jets");
  jetTree->GetBranch("dEtaJetsLO")
      ->SetTitle("Eta difference of the two leading jets");
  jetTree->GetBranch("alpha")->SetTitle(
      "If there are at least 3 jets in the event, alpha is the third jet's "
      "transverse momentum divided by the average transverse momentum of the "
      "two leading jets");
  ...
  jetTree->GetBranch("pthat")->SetTitle(
      "Transverse momentum of the generated hard process");
  jetTree->GetBranch("eventWeight")->SetTitle("Monte Carlo generator weight");
  jetTree->GetBranch("rhoAll")->SetTitle(
      "The median density (in GeV/A) of pile-up contamination per event; "
      "computed from all PF candidates");
  jetTree->GetBranch("rhoCentral")
      ->SetTitle("The median density (in GeV/A) of pile-up contamination per "
                 "event; computed from all PF candidates with |eta|<2.5");
  jetTree->GetBranch("rhoCentralNeutral")
      ->SetTitle(
          "The median density (in GeV/A) of pile-up contamination per event; "
          "computed from all neutral PF candidates with |eta| < 2.5");
  jetTree->GetBranch("rhoCentralChargedPileUp")
      ->SetTitle("The median density (in GeV/A) of pile-up contamination per "
  ...
  jetTree->GetBranch("Pileup_nPU")
      ->SetTitle("The number of pileup interactions that have been added to "
                 "the event in the current bunch crossing");
  jetTree->GetBranch("Pileup_nTrueInt")
      ->SetTitle("The true mean number of the poisson distribution for this "
                 "event from which the number of interactions in each bunch "
                 "crossing has been sampled");
}
...
void JetAnalyzer::analyze(const edm::Event &iEvent,
  ...
  iEvent.getByToken(pfRhoCentralChargedPileUpToken_,
                    pfRhoCentralChargedPileUpHandle);
  ...
  for (pat::PackedCandidateCollection::const_iterator candIt = pfs->begin(); candIt != pfs->end(); candIt++){
		  ...
		  pfCandVx.push_back(cand.vx());
		  pfCandVy.push_back(cand.vy());
		  pfCandVz.push_back(cand.vz());
  ...
  // sortedJets include all jets of the event, while selectedJets have pT and
  // eta cuts
  ...
  int iJetR = -1;
  for (pat::JetCollection::const_iterator jetIt = jets->begin();
  ...
  for (unsigned int ptIdx = 0; ptIdx < selectedJets.size(); ++ptIdx) {
    ...
    if (abs(j.eta()) <= 2.7 && (numconst > 1 && nhf < 0.99 && nemf < 0.99) &&
        ((abs(j.eta()) <= 2.4 && chf > 0 && chm > 0 && cemf < 0.99) ||
         abs(j.eta()) > 2.4)) {
      jetLooseID = 1;
    ...
    // Add variables for deltaPhi and deltaEta for the two leading jets of the
    // event
    dPhiJetsLO = deltaPhi(sortedJets[0].jet.phi(), sortedJets[1].jet.phi());
    dEtaJetsLO = sortedJets[0].jet.eta() - sortedJets[1].jet.eta();

    // The alpha variable is the third jet's pT divided by the average of the
    // two leading jets' pT
    alpha = 0;
    // Make sure that there are at least 3 jets in the event
    if (sortedJets.size() > 2) {
      Float_t leadingPtAvg =
          (sortedJets[0].jet.pt() + sortedJets[1].jet.pt()) * 0.5;
    ...
    physFlav = 0;
    if (j.genParton())
      physFlav = j.genParton()->pdgId();

    // For convenience, save variables distinguishing gluon, light quark and
    // other jets
    isPartonUDS = 0;
    isPartonG = 0;
    isPartonOther = 0;
    ...
    edm::RefToBase<pat::Jet> jetRef(
        edm::Ref<pat::JetCollection>(jets, iJetRef));
    ...
    int vtxGood = 0;
    for (VertexCollection::const_iterator i_vtx = vertices->begin();
         i_vtx != vertices->end(); i_vtx++) {
      if (!(i_vtx->isFake()) && i_vtx->ndof() > goodVtxNdof &&
          fabs(i_vtx->z()) <= goodVtxZ &&
          fabs(i_vtx->position().Rho()) <= goodVtxRho) {
        vtxGood++;
      }
    }
    PV_npvsGood = vtxGood;
    ...
    // Loop over the PF candidates contained inside the jet, first sorting them
    // in pT order
    std::vector<reco::CandidatePtr> pfCands = j.daughterPtrVector();
    std::sort(pfCands.begin(), pfCands.end(),
    ...
    for (unsigned int i = 0; i < pfCandsSize; ++i) {
      const pat::PackedCandidate &pf =
          dynamic_cast<const pat::PackedCandidate &>(*pfCands[i]);
      const pat::PackedCandidate *pfPointer = &pf;
      pfMap.insert(
          std::pair<const pat::PackedCandidate *, const pat::PackedCandidate>(
              pfPointer, pf));
    ...
    if (j.genJet()) {
      ...
      for (unsigned int i = 0; i < gj->numberOfDaughters(); ++i) {
        const pat::PackedGenParticle *genParticle =
            dynamic_cast<const pat::PackedGenParticle *>(gj->daughter(i));
        genParticles.push_back(genParticle);
      }

      // Sort the constituents in pT order
      std::sort(
          genParticles.begin(), genParticles.end(),
          [](const pat::PackedGenParticle *p1,
             const pat::PackedGenParticle *p2) { return p1->pt() > p2->pt(); });

      unsigned int genParticlesSize = genParticles.size();
      for (unsigned int i = 0; i != genParticlesSize; ++i) {
        const pat::PackedGenParticle *genParticle =
            dynamic_cast<const pat::PackedGenParticle *>(genParticles[i]);

        float dEta = (genParticle->eta() - gj->eta());
        float dPhi = deltaPhi(genParticle->phi(), gj->phi());

        genJetPF_pT[ng] = genParticle->pt();
        genJetPF_dR[ng] = deltaR(gj->eta(), gj->phi(), genParticle->eta(),
                                 genParticle->phi());
    ...
    //  within the area of |deltaEta| < 1 & |deltaPhi| < 1 from the center of
    //  the jet
    if (!(kMaxPF < pfs->size()))
    ...
    for (unsigned int i = 0; i != pfsSize; ++i) {
      ...
      if ((fabs(dEta) > 1.0) || (fabs(dPhi) > 1.0))
        continue;
      PF_pT[npfs] = pf.pt();
